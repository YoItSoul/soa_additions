...
/** Perfectionist — tool: round outgoing to 10. armor: round incoming>5 to nearest 5. */
public class PerfectionistModifier extends Modifier implements MeleeDamageModifierHook, ModifyDamageModifierHook {
    @Override
    ...
    public float getMeleeDamage(IToolStackView tool, ModifierEntry mod, ToolAttackContext ctx, float base, float dmg) {
        return Math.round(dmg / 10.0F) * 10.0F;
    }
    @Override
    public float modifyDamageTaken(IToolStackView tool, ModifierEntry mod, EquipmentContext ctx,
        ...
        return dmg > 5.0F ? Math.round(dmg / 5.0F) * 5.0F : dmg;
    }
}
